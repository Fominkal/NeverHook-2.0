...
MinimalistUILib.Tabs = {}

-- Инициализация UI (создаем все основные элементы)
function MinimalistUILib.Init()
    -- Создаем ScreenGui
...
-- Создание вкладки, возвращаем Frame для элементов
function MinimalistUILib.CreateTab(name)
    assert(TabBar and ContentFrame, "UI не инициализирован. Вызови MinimalistUILib.Init() сначала.")
    ...
    MinimalistUILib.Tabs[name] = { Button = tabButton, Content = tabContent }

    tabButton.MouseButton1Click:Connect(function()
        for _, tab in pairs(MinimalistUILib.Tabs) do
...
-- Создание чекбокса с callback
function MinimalistUILib.CreateCheckbox(parent, text, callback)
...
-- Создание слайдера с callback
function MinimalistUILib.CreateSlider(parent, text, min, max, default, callback)
...
-- Создание бинда с установкой и вызовом
function MinimalistUILib.CreateBind(parent, text, defaultKey, onBindSet, onToggle)
    local container = Instance.new("Frame", parent)
    container.Size = UDim2.new(1, 0, 0, 30)
    container.BackgroundTransparency = 1
    ...
    label.BackgroundColor3 = AccentColor
    ...
    local function toggleBind()
        ...
        label.Text = text .. ": " .. bindKey.Name .. (enabled and " (ON)" or " (OFF)")
    end
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if waitingForKey and input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            waitingForKey = false
            if onBindSet then onBindSet(bindKey) end
            label.Text = text .. ": " .. bindKey.Name
...
-- Функция скрытия / показа UI
function MinimalistUILib.SetVisible(visible)
    if ScreenGui then
        ScreenGui.Enabled = visible
    end
end

return MinimalistUILib
